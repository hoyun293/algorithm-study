...
for(let i = 1 ; i <= N; i++){
    ...
    if(order === 'push_front'){
        ...
        rslt += (dequeue.length === 0 ? -1 : dequeue.shift()) + '\n';
    }else if(order === 'pop_back'){
        rslt += (dequeue.length === 0 ? -1 : dequeue.pop()) + '\n';
    }else if(order === 'size'){
        rslt += dequeue.length + '\n';
    }else if(order === 'empty'){
        rslt += (dequeue.length === 0 ? 1 : 0) + '\n'; 
    }else if(order === 'front'){
        rslt += (dequeue.length === 0 ? -1 : dequeue[0]) + '\n';
    }else if(order === 'back'){
        rslt += (dequeue.length === 0 ? -1 : dequeue[dequeue.length-1]) + '\n';
    }
}
